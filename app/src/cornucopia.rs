...
pub mod queries {
    pub mod fortunes {
        ...
        pub struct FortunesBorrowed<'a> {
            pub id: i32,
            pub message: &'a str,
        }
        impl<'a> From<FortunesBorrowed<'a>> for Fortunes {
            fn from(FortunesBorrowed { id, message }: FortunesBorrowed<'a>) -> Self {
                Self {
                    id,
        ...
        pub struct FortunesQuery<'a, C: GenericClient, T, const N: usize> {
            ...
            extractor: fn(&tokio_postgres::Row) -> FortunesBorrowed,
        ...
        where
            C: GenericClient,
        {
            pub fn map<R>(self, mapper: fn(FortunesBorrowed) -> R) -> FortunesQuery<'a, C, R, N> {
                FortunesQuery {
                    client: self.client,
                    params: self.params,
                    stmt: self.stmt,
...
FROM 
    Fortune"))
        ...
        pub struct FortunesStmt(cornucopia_async::private::Stmt);
        impl FortunesStmt {
            pub fn bind<'a, C: GenericClient>(
                &'a mut self,
                client: &'a C,
            ) -> FortunesQuery<'a, C, Fortunes, 0> {
                FortunesQuery {
                    client,
                    params: [],
                    stmt: &mut self.0,
                    extractor: |row| FortunesBorrowed {
                        id: row.get(0),
                        message: row.get(1),
                    },
                    mapper: |it| <Fortunes>::from(it),
                }
            }
        }
    }
